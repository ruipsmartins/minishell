...
/*   Updated: 2024/10/21 18:39:19 by ruidos-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void execute_piped_commands(t_command *cmd, t_data *data)
{
    ...
    while (cmd != NULL)
    {
        ...
        if (pid == 0)
        {
            ...
            ft_child(in_fd, cmd, fd, data, exit_pipe); // Passa o novo pipe para ft_child
            exit(EXIT_SUCCESS);   // Sai do filho após executar o comando
        }
        else if (pid < 0)
        {
            perror("fork error:");
...
int ft_child(int in_fd, t_command *cmd, int fd[2], t_data *data, int exit_pipe[2])
{
    ...
    if (data->close_shell) 
    {
        write(exit_pipe[1], "1", 1); // Escreve '1' para indicar que o comando 'exit' foi encontrado.
    }
